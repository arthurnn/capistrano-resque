      ...
      capistrano_config.load do
        ...
        namespace :resque do
          ...
          task :status, :roles => lambda { workers_roles() }, :on_no_matching_servers => :continue do
            command = "if [ -e /www/500px/tmp/pids/resque_work_1.pid ]; then \
...
PIDFILE=./tmp/pids/scheduler.pid BACKGROUND=yes #{fetch(:bundle_cmd, "bundle")} exec rake resque:scheduler >> #{shared_path}/log/resque_scheduler.log 2>&1 &"
            end
